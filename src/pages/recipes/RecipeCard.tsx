...
const RecipeCard = ({ recipe }: { recipe: RecipeType }) => {
  return (
    <div className="max-w-sm rounded overflow-hidden shadow-lg m-2 flex flex-col justify-between h-full">
      <div>
        <Image src={recipe.image} alt={recipe.name} width={400} height={400} className="w-full" />
      ...
      <div className="px-6 pt-4 pb-2 mb-4">
        <Link href={`/recipes/${recipe.id}`} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded w-full text-center block">
          View Recipe
        </Link>
      </div>
    </div>
